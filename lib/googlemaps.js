...
exports.placeSearch = function(query, key, callback, sensor, lang) {
  var args = {
    query: query,
    key: key
  };
...
exports.placeZIPSearch = function(query, countryCode, key, callback, sensor, lang) {
  var args = {
    input: query,
    key: key,
    types: '(regions)',
...
exports.timezone = function(location, timestamp, callback, sensor, language) {
  if ('object' === typeof location) {
    location = exports.checkAndConvertPoint(location);
  }
  ...
  return makeRequest(path, args, true, returnObjectFromJSON(callback), 'binary');
};
  ...
  switch (typeof input) {
    case 'object':
      if (input instanceof Array) {
        ...
        return input.lat.toString() + ',' + input.lng.toString();
      }
      break;
    case 'string':
      return input;
...
function buildUrl(path, args) {
  if (config('google-client-id') && config('google-private-key') && typeof args.key === 'undefined') {
